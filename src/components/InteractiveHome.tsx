...
// NOTE(Agent): This component was extracted from page.tsx (which was a monolithic
// 'use client' page) to enable page.tsx to be a React Server Component.
// Result: The browser receives actual HTML content on first load instead of an empty page,
// dramatically improving LCP by allowing the browser to paint content without waiting for JS.
// All browser-API-dependent logic (localStorage, fetch, searchParams hooks) lives here.

import { useState, useEffect, useCallback, useRef, useMemo, Suspense, useTransition } from 'react'
...
const PermitList = dynamic(() => import('@/components/PermitList'))
const PaymentModal = dynamic(() => import('@/components/PaymentModal'))
const PermitDetailModal = dynamic(() => import('@/components/PermitDetailModal'))

// Leaflet requires browser APIs — load client-side only
// NOTE(Agent): The loading skeleton reserves map dimensions to prevent CLS during Leaflet init.
const Map = dynamic(() => import('@/components/Map'), {
    ssr: false,
    loading: () => (
        <div
            className="flex-1 min-h-[50vh] lg:min-h-0"
            style={{ backgroundColor: 'var(--background-primary)', opacity: 0.5 }}
...
function HomeContent() {
    ...
    if (!search) {
        return (
            <main id="main-content" style={{ backgroundColor: 'var(--background-primary)' }}>
    ...
    return (
        <main id="main-content" className="flex flex-col h-screen" style={{ backgroundColor: 'var(--background-primary)' }}>
            {/* Header — Dark editorial glass */}
            <header
                className="glass px-4 py-4 z-10 relative"
                aria-label="CivicScout site header"
                style={{
            ...
            <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
                ...
                {(search || permits.length > 0) && (
                    <aside
                        ...
                        aria-label="Permit results and filters"
                        style={{
                    ...
                    >
                        {search && (
                            ...
                            >
                                ...
                                {/* NOTE(Agent): min-height reserves space so conditional messages
                    (stale data warning, unsupported area) don't shift the permit
                    list below them — fixing CLS on data-load state changes. */}
                        ...
                        {permits.length > 0 && (
                            <SeverityFilter
                                value={minSeverity}
                                onChange={(sev) => {
                                    // NOTE(Agent): startTransition marks the filter re-render as
                                    // non-urgent so interactions (click, scroll) can interrupt it.
                                    startFilterTransition(() => setMinSeverity(sev))
                                }}
                                counts={{
                        ...
                        {/* NOTE(Agent): isFilterPending from useTransition is available for
                a future loading indicator on the PermitList during filter transitions. */}
                        <PermitList
                            permits={filteredPermits}
                            selectedPermitId={selectedPermitId}
                        ...
                        <div className="mt-12 pt-8 border-t" style={{ borderTopColor: 'var(--border-strong)' }}>
                            ...
                            <div className="space-y-3">
                                <a
                                    ...
                                    className="flex items-start gap-3 group rounded-lg p-2.5 -mx-2.5 transition-colors hover:bg-[rgba(10,158,142,0.06)]"
                                >
                                    <div
                                    ...
                                    >
                                        <span className="text-xs" role="img" aria-hidden="true">🏙️</span>
                                    </div>
                                    <div>
                                        <p
                                ...
                                {cityName === 'Chicago' && (
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="flex items-start gap-3 group rounded-lg p-2.5 -mx-2.5 transition-colors hover:bg-[rgba(27,155,108,0.06)]"
                                    >
                                        <div
                                            className="p-1.5 rounded-md"
                                            style={{
...
// NOTE(Agent): Wrap in Suspense here (inside the client component tree) because
// useSearchParams() requires a Suspense boundary. The skeleton fallback in page.tsx
// (the Server Component) provides the initial paint while this client bundle loads.
